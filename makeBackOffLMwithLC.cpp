...
int main(int argc, char *argv[]) {
    ...
    for (vector<arparow>::iterator it = unigram.begin(); it != unigram.end(); ++it) {
        ...
        lm_fst.AddArc(from_id, LexArc(syms->Find(it->gram.c_str()), syms->Find(it->gram.c_str()), LexWeight(0.0, it->logprob), to_id));
        //lm_fst.AddArc(to_id, fst::StdArc(0, 0, it->bkprob, from_id));
        lm_fst.AddArc(to_id, LexArc(0, 0, LexWeight(0.0, it->bkprob), from_id));
        if (boost::iequals(it->gram, string("</s>"))) {
            //lm_fst.SetFinal(to_id, 0.0);
            lm_fst.SetFinal(to_id, LexWeight(0.0, 0.0));
        }
    }
    ...
    for (vector<arparow>::iterator it = bigram.begin(); it != bigram.end(); ++it) {
        ...
        lm_fst.AddArc(from_id, LexArc(syms->Find(ng2.c_str()), syms->Find(ng2.c_str()), LexWeight(0.0, it->logprob), to_id));
        //lm_fst.AddArc(to_id, fst::StdArc(0, 0, it->bkprob, return_id));
        lm_fst.AddArc(to_id, LexArc(0, 0, LexWeight(0.0, it->bkprob), return_id));
    ...
    for (vector<arparow>::iterator it = trigram.begin(); it != trigram.end(); ++it) {
        ...
        lm_fst.AddArc(from_id, LexArc(syms->Find(ng3.c_str()), syms->Find(ng3.c_str()), LexWeight(0.0, it->logprob), to_id));
        int bk_to_id = lm_id.find(ng3)->second;
        //lm_fst.AddArc(to_id, fst::StdArc(0, 0, 0.0, bk_to_id));
        lm_fst.AddArc(to_id, LexArc(0, 0, LexWeight(0.0, 0.0), bk_to_id));
    ...
    lm_fst.AddArc(from_id, LexArc(0, 0, LexWeight(0.0, 0.0), to_id));
    lm_fst.Write("data/cpplm.lex.fst");
}
